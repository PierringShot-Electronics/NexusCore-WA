    ...
    try {
        for (const row of rows) {
         ...
         RETURNING id`, [
            ...
            if (!productRow) {
                logger_1.logger.warn({ sku, name }, 'Skipping product upsert without returned id');
                continue;
            }
            if (!openai) {
                continue;
           ...
           VALUES ($1, $2)
           ON CONFLICT (product_id) DO UPDATE SET embedding = EXCLUDED.embedding`, [productRow.id, (0, pg_1.toSql)(embedding.embedding)]);
            }
        }
